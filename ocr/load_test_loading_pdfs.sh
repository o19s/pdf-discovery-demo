...
UPLOAD_ENDPOINT=http://localhost:8983/solr/documents/update/extract

echo "Loading files in $PATH_TO_SAMPLE_DOCS"

for filename in $PATH_TO_SAMPLE_DOCS/*.pdf; do
  echo "Posting ${filename}"
  curl "http://localhost:8983/solr/documents/update/extract?commit=false&literal.id=${filename}" -F "myfile=@${filename}"
done
